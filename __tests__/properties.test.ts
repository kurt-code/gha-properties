...
describe('properties.ts', () => {
  ...
  describe('getPropertiesForKeys', () => {
    it('should return properties for specified keys', async () => {
      ...
      const result = await getPropertiesForKeys('test.properties', [
        'key1',
        'key3'
      ])
      console.log(result)
  ...
  describe('writePropertyToFile', () => {
    ...
    it('should handle file write error', async () => {
      ...
      await expect(
        writePropertyToFile('test.properties', 'key1', 'value1')
  ...
  describe('writePropertiesToFile', () => {
    ...
    it('should handle file write error', async () => {
      ...
      jest.spyOn(fs.promises, 'readFile').mockRejectedValueOnce(error)
      await expect(
        writePropertiesToFile('test.properties', { key1: 'value1' })
      ).rejects.toThrow(error)
    })
  })
})
